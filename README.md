...
| Extractor | Fallback only — built with [coding agent](#how-i-use-ai-tools-in-development) | CSS selectors cover ~95% of pages; LLM only fires when the DOM returns nothing, avoiding cost on the hot path. Claude Code wrote the selector logic using `products_example_output.csv` as ground truth, verifying extracted values matched known-correct field values. |
